...
meta <- readxl::read_xlsx("D:/KSY/Project/2.DT_Swab[논문작업pf남경화]/Paper/HN00182797_Original_Sample_QC_and_NGS_Library_QC.xlsx")
...
df2 <- merge(df, meta[, c("Sample_Name", "Original_QC_Conc_ng_uL")], by.x = "sample_id", by.y = "Sample_Name")
```
...
kruskal.test(Depth ~ swab_type2, data = df2)
dunn.test(df2$Depth, df2$swab_type2, method = "bh")  # BH = FDR
# 
# 	Kruskal-Wallis rank sum test
# 
# data:  Depth by swab_type2
# Kruskal-Wallis chi-squared = 1.0739, df = 4, p-value = 0.8984
# 
#   Kruskal-Wallis rank sum test
# 
# data: x and group
# Kruskal-Wallis chi-squared = 1.4974, df = 4, p-value = 0.83
# 
# 
...
# 
# Col Mean-│
# Row Mean │      Copan      Eswab       Omni   Puritan_
# ─────────┼────────────────────────────────────────────
#    Eswab │   0.000000
#          │     0.5000 
#          │
#     Omni │   0.843661   0.843661
#          │     0.6648     0.9971 
#          │
# Puritan_ │   0.168732   0.168732  -0.674929
#          │     0.4811     0.5413     0.6247 
#          │
# Puritan_ │  -0.322125  -0.322125  -1.165786  -0.490858
#          │     0.5338     0.6228     1.0000     0.6235 
# 
# FDR = 0.05
# Reject Ho if adjusted p ≤ FDR/2 with stopping rule, where (unadjusted) p = Pr(Z ≥ |z|)
kruskal.test(Original_QC_Conc_ng_uL ~ swab_type2, data = df2)
dunn.test(df2$Original_QC_Conc_ng_uL, df2$swab_type2, method = "bh")
#   Kruskal-Wallis rank sum test
# 
# data: x and group
# Kruskal-Wallis chi-squared = 6.1919, df = 4, p-value = 0.19
# 
# 
#                     Dunn's Pairwise Comparison of x by group                    
#                               (Benjamini-Hochberg)                              
...
#          │     0.1642 
#          │
#     Omni │  -0.377484   1.756457
#          │     0.5042     0.1317 
#          │
# Puritan_ │  -0.500744   1.633196  -0.123260
#          │     0.6166     0.1280     0.5011 
#          │
# Puritan_ │  -0.030815   2.103126   0.346669   0.469929
#          │     0.4877     0.0886     0.4555     0.5320 
# 
# FDR = 0.05
# Reject Ho if adjusted p ≤ FDR/2 with stopping rule, where (unadjusted) p = Pr(Z ≥ |z|)
...
# Antecubital fossa만
df2.ac <-  df2 %>% filter(site_group == "Antecubital")
kruskal.test(Depth ~ swab_type2, data = df2.ac)
kruskal.test(Original_QC_Conc_ng_uL ~ swab_type2, data = df2.ac)
dunn.test(df2.ac$Depth, df2.ac$swab_type2, method = "bh")  # df 사용 (오타로 보임)
# Forehead만
df2.fh <-  df2 %>% filter(site_group == "Forehead")
kruskal.test(Depth ~ swab_type2, data = df2.fh)
kruskal.test(Original_QC_Conc_ng_uL ~ swab_type2, data = df2.fh)
dunn.test(df2.fh$Depth, df2.fh$swab_type2, method = "bh")  # df 사용 (오타로 보임)
